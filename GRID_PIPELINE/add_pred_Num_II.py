...
def mean_num(chunk):
    ...
    for it in new_array_0:
        ...
        sumit_0.append(tmp_2.iloc[0].NumMosquitos)
    for it in new_array_1:
        tmp_2 = tmp_1[tmp_1['HexCell']==str(it)]
        try:
            sumit_1.append(tmp_2.iloc[0].NumMosquitos)
        except:
            pass
    for it in new_array_2:
        tmp_2 = tmp_1[tmp_1['HexCell']==str(it)]
        try:
            sumit_2.append(tmp_2.iloc[0].NumMosquitos)
        except:
            pass
        
    chunk['Mean_Num_Lvl_0'] = np.mean(sumit_0)
    chunk['Min_Num_Lvl_0'] = np.min(sumit_0)
    chunk['Max_Num_Lvl_0'] = np.max(sumit_0)
    chunk['Mean_Num_Lvl_1'] = np.mean(sumit_1)
    chunk['Min_Num_Lvl_1'] = np.min(sumit_1)
    chunk['Max_Num_Lvl_1'] = np.max(sumit_1)
    chunk['Mean_Num_Lvl_2'] = np.mean(sumit_2)
    chunk['Min_Num_Lvl_2'] = np.min(sumit_2)
    chunk['Max_Num_Lvl_2'] = np.max(sumit_2)
    ...
    if chunk.iloc[0].Calender_Week > 19:
        ...
        chunk['Mean_Num_Lvl_0_cwbef'] = np.mean(sumit_0)
        chunk['Min_Num_Lvl_0_cwbef'] = np.min(sumit_0)
        chunk['Max_Num_Lvl_0_cwbef'] = np.max(sumit_0)
        chunk['Mean_Num_Lvl_1_cwbef'] = np.mean(sumit_1)
        chunk['Min_Num_Lvl_1_cwbef'] = np.min(sumit_1)
        chunk['Max_Num_Lvl_1_cwbef'] = np.max(sumit_1)
        chunk['Mean_Num_Lvl_2_cwbef'] = np.mean(sumit_2)
        chunk['Min_Num_Lvl_2_cwbef'] = np.min(sumit_2)
        chunk['Max_Num_Lvl_2_cwbef'] = np.max(sumit_2)
        
    else:
        chunk['Mean_Num_Lvl_0_cwbef'] = 0
        chunk['Min_Num_Lvl_0_cwbef'] = 0
        chunk['Max_Num_Lvl_0_cwbef'] = 0
        chunk['Mean_Num_Lvl_1_cwbef'] = 0
        chunk['Min_Num_Lvl_1_cwbef'] = 0
        chunk['Max_Num_Lvl_1_cwbef'] = 0
...
"""
df_num = pd.read_csv('COMP/comp_hex_num_II.csv')  
...
if __name__ == '__main__':
    ...
    df_train.to_csv("TRAIN/train_hex_pred_II.csv", index=False)
    df_comp.to_csv("COMP/comp_hex_pred_II.csv", index=False)
    df_test.to_csv("TEST/test_hex_pred_II.csv", index=False)
